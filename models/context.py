    ...
    def __init__(self, planes, scale, reduce_ratio_nl, att_mode='origin'):
        ...
        if att_mode == 'origin':
            ...
            self.conv_att = nn.Sequential(
                ...
                nn.ReLU(True),
                antialiased_cnns.BlurPool(planes//4,stride=1),
...
class AGCB_Element(nn.Module):
    def __init__(self, planes, scale=2, reduce_ratio_nl=32, att_mode='origin'):
        ...
        self.relu = nn.Sequential(nn.ReLU(True),antialiased_cnns.BlurPool(planes,stride=1))
        #self.relu = nn.ReLU(True)
...
class CPM(nn.Module):
    def __init__(self, planes, block_type, scales=(3,5,6,10), reduce_ratios=(4,8), att_mode='origin'):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.ReLU(True),
            antialiased_cnns.BlurPool(inter_planes,stride=1)
        )
        ...
        self.conv2 = nn.Sequential(
            ...
            nn.BatchNorm2d(planes),
            nn.ReLU(True),
            antialiased_cnns.BlurPool(planes,stride=1)
        )

    def forward(self, x):
        
        reduced = self.conv1(x)
        #mid_feature.append(reduced)  #不含抗混叠的特征
        blocks = []
